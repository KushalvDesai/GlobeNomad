...
import { MapPin, Plus, ArrowRight, RefreshCw } from "lucide-react";
...
export function PreviousTrips() {
  ...
  const { data, loading, error, refetch, networkStatus } = useQuery<{ myTrips: TripsResponse }>(GET_MY_TRIPS, {
    variables: { limit: 6.0, offset: 0.0 }, // Changed to float values
    errorPolicy: "all",
    ...
    <section className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {showingDbTrips && (
            <span className="px-2 py-1 text-xs rounded-full bg-green-500 text-white">
              {data?.myTrips?.total || 0} from database
            </span>
          )}
          {error && (
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-md bg-[var(--accent-1)] text-[var(--foreground)] hover:bg-[var(--accent-1)]/90 flex items-center gap-2 text-sm font-medium transition-colors"
          >
      ...
      {showingDbTrips && (
        <div className="text-green-400 text-sm">
          ✅ Showing your recent trips from the database
        </div>
      )}

      {!showingDbTrips && !error && (
        <div className="text-yellow-400 text-sm">
          ⚠️ Showing fallback popular destinations
        </div>
      )}
